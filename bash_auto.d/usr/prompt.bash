...
# one unit.
[ -r /proc/loadavg ] && [ "$(cut -d'.' -f1 /proc/loadavg)" -ge 1 ] && \
...
#    prompt="\[\e[32;1m\]<\u@\h> \[\e[34;1m\][\w][31;1m\\$[0;0m\[\e[0m\] "
...
# Put titlebar in xterms if supported. We use property of chained
# prompt variable (prompt="something$prompt"), and we like a vim
# titlebar style (user@host + (dir)).
[[ "$TERM" = xterm* ]] && \
	prompt="\[\e]2;\u@\h + (\w)\a\]$prompt"
